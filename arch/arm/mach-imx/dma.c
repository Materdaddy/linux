...
static irqreturn_t dma_irq_handler(int, void *, struct pt_regs *);
#else
static irqreturn_t imx21_dma_irq_handler(int, void *, struct pt_regs *);
#endif
...
int imx_dma_request(imx_dmach_t dma_ch, const char *name)
{
	...
	local_irq_restore(flags);
        ...
        if (request_irq(DMA_INT(dma_ch), 
                        imx21_dma_irq_handler, 0, "DMA", NULL)) {
		printk(KERN_CRIT "Wow!  Can't register IRQ for DMA %d\n",
                       dma_ch);
		return -ENODEV;
	}
...
/*
 * IMX and IMX21 handle DMA interrupts differently.
 * On IMX, there is one interrupt for all channels, plus
 *   another error interrupt for all channels. 
 * On IMX21, each channel has its own interrupt.
 */
#if defined(CONFIG_ARCH_IMX)
...
#warning "DMA errors are not handled."

static irqreturn_t
imx21_dma_irq_handler(int irq, void *dev_id, struct pt_regs *regs)
{
        int ch = irq - DMA_INT(0);
        struct imx_dma_channel *channel = &imx_dma_channels[ch];

	pr_debug(KERN_DEBUG 
                 "imxdma: imx21_dma_irq_handler called, disr=0x%08x\n", 
                 disr);

        DISR = (1 << ch);

        if (channel->name) {
            if(imx_dma_sg_next(ch, CNTR(ch))){
                CCR(ch) &= ~CCR_CEN;
                mb();
                CCR(ch) |= CCR_CEN;
            }else{
                if(channel->irq_handler)
                    channel->irq_handler(ch, channel->data, regs);
            }
        } else {
            /*
             * IRQ for an unregistered DMA channel:
