/*
 * arch/arm/mach-imx21/csb536fs.c
 *
 * Initially based on:
...
static int csb536_mmc_card_present(void)
{
    if (SSR(1) & (1 << 16)) {
        return 0;
    } else {
...
static void csb536_mmc_setpower(struct device *dev, unsigned int vdd)
{
    if (vdd) {
//        printk("%s: on\n", __FUNCTION__);
...
static struct imxmmc_platform_data csb536_mmc_info = {
    .card_present   = csb536_mmc_card_present,
    .setpower      = csb536_mmc_setpower,
};
...
csb536_init(void)
{
    ...
    imx_set_mmc_info(&csb536_mmc_info);
...
static struct map_desc csb536_io_desc[] __initdata = {
    ...
    {IMX_CS2_VIRT, __phys_to_pfn(IMX_CS2_PHYS), IMX_CS2_SIZE, MT_DEVICE},
    {IMX_CS3_VIRT, __phys_to_pfn(IMX_CS3_PHYS), IMX_CS3_SIZE, MT_DEVICE},
    {IMX_CS4_VIRT, __phys_to_pfn(IMX_CS4_PHYS), IMX_CS4_SIZE, MT_DEVICE},
    {IMX_CS5_VIRT, __phys_to_pfn(IMX_CS5_PHYS), IMX_CS5_SIZE, MT_DEVICE},
/*   {IMX_FB_VIRT,  __phys_to_pfn(IMX_FB_PHYS),  IMX_FB_SIZE,  MT_DEVICE}, */
};
...
/*	MAINTAINER("Jay Monkman <jtm@lopingdog.com>") */
	...
	.boot_params	= 0x08000000,

	.map_io		= csb536_map_io,
