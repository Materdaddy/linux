 ...
 *
 * Touchscreen driver for ADS7846 
 *
...
#define DRIVER_DESC	"ADS7846 touchscreen driver"
...
static irqreturn_t ads7846_spi_isr(int irq, void *dummy, struct pt_regs *fp)
{
    ...
    SSP_INT_REG(0) &= ~SSP_INT_TEEN;

    while (SSP_CTRL_REG(0) & SSP_XCH) continue;

    n = (SSP_TEST_REG(0) >> 4) & 0xf;
    if (n != 3) {
        printk("Error reading SPI - 3 words expected, got %d\n", n);
    }

    junk = (SSP_RX_REG(0) & 0xff);
    t0 = (SSP_RX_REG(0) & 0xff);
    t1 = (SSP_RX_REG(0) & 0xff);
...
static void ads7846_acquire(void)
{
    ...
    while (SSP_TEST_REG(0) & 0xf0) {
        printk("%s:%d :: %d words in RX fifo\n", 
               __FUNCTION__, __LINE__, (SSP_TEST_REG(0) >> 4) & 0xf);
	junk = SSP_RX_REG(0);
    }
...
//    printk("%s\n", DRIVER_DESC);
    ...
    imx_gpio_mode(18 | GPIO_PORTB | GPIO_IN | GPIO_PUEN | GPIO_GPIO);     /* *PENIRQ */
    ...
    /* Install ISR */
    rc = request_irq(INT_CSPI1, &ads7846_spi_isr, 0, "ADS7846-SPI", NULL);
    if (rc < 0) {
...
#elif defined(CONFIG_MACH_CSB535)
    /* Configure GPIO */
    imx_gpio_mode(29 | GPIO_PORTD | GPIO_OUT);    /* SCLK */
    imx_gpio_mode(28 | GPIO_PORTD | GPIO_OUT);    /* *SS */
    imx_gpio_mode(30 | GPIO_PORTD | GPIO_IN);     /* MISO */
    imx_gpio_mode(31 | GPIO_PORTD | GPIO_OUT);    /* MOSI */

    imx_gpio_mode(1 | GPIO_PORTE | GPIO_IN | GPIO_PUEN | GPIO_GPIO);     /* *PENIRQ */
    ...
    SSP_CTRL_REG(0) |= SSP_XCH;
    while ((SSP_CTRL_REG(0) & SSP_XCH) || (SSP_INT_REG(0) & SSP_INT_RR)) {
        tmp = SSP_RX_REG(0);
    }
    ...
    ads.input->name = "ADS7846 Touchscreen";
