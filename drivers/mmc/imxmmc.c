...
#define IMXMCI_INT_MASK_DEFAULT (INT_MASK_BUF_READY | INT_MASK_DATA_TRAN | \
		      ...
		      INT_MASK_AUTO_CARD_DETECT | INT_MASK_DAT0_EN | \
                      INT_MASK_SDIO)
#else
#define IMXMCI_INT_MASK_DEFAULT (INT_MASK_BUF_READY | INT_MASK_DATA_TRAN | \
	              INT_MASK_WRITE_OP_DONE | INT_MASK_END_CMD_RES | \
		      INT_MASK_DAT0_EN | INT_MASK_SDIO)
#endif
...
#if defined(CONFIG_ARCH_IMX)  /* IMX21 doesn't need this. */
...
#else
	MMC_INT_MASK = host->imask | INT_MASK_SDIO;
#endif
...
#else
		if (req->data->flags & MMC_DATA_STREAM) {
                    printk("req->data->flags & MMC_DATA_STREAM\n");
		}
#endif /* defined(CONFIG_ARCH_IMX) */
	}
...
#elif defined(CONFIG_ARCH_IMX21)
		imx_gpio_mode(GPIO_PORTE|GPIO_IN | GPIO_PUEN | GPIO_GPIO | 21);
#endif /* defined(CONFIG_ARCH_IMX21) */
	}
...
#elif defined(CONFIG_ARCH_IMX21)
	imx_gpio_mode(PE18_PF_SD_DAT0);
	imx_gpio_mode(PE19_PF_SD_DAT1);
	imx_gpio_mode(PE20_PF_SD_DAT2);
	/* Configured as GPIO with pull-up to ensure right MCC card mode */
	/* Switched to PB11_PF_SD_DAT3 if 4 bit bus is configured */
	imx_gpio_mode(GPIO_PORTE | GPIO_IN | GPIO_PUEN | GPIO_GPIO | 21);
	/* imx_gpio_mode(PB11_PF_SD_DAT3); */
	imx_gpio_mode(PE23_PF_SD_CLK);
	imx_gpio_mode(PE22_PF_SD_CMD);
#endif /* defined(CONFIG_ARCH_IMX21) */

	imxmci_softreset();

	if ( (MMC_REV_NO != 0x390) && (MMC_REV_NO != 0x400) ) {
