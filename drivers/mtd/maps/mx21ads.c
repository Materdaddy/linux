 ...
 * @file mx21_flash.c
 *
 * @brief This file contains the MTD Mapping information on the mx21 ADS.
 *
 * @ingroup NOR_MTD
 */
...
/*!
 * The PA of the NOR flash memory base on the ADS board at CS0
 */
#define MX21_FLASH_BASE_PA       0xC8000000
/*!
 * The size in bytes for the NOR flash memory on the ADS board
 */
#define MX21_FLASH_SIZE          0x02000000

/*!
 * ADS NOR flash memory information
 */
static struct map_info mx21_map = {
        .name      =    "flash00",
        .size      =    MX21_FLASH_SIZE,
        .phys      =    MX21_FLASH_BASE_PA,
        .bankwidth =    4,
};

/*!
 * This table contains the static partition information for the ADS
 ...
 */
static struct mtd_partition mx21_part[] =
{
        {
                .name       =   "Bootloader",
                .size       =   1* 1024 * 1024,
 ...
 * and used throughout the rest of the mx21_mtd_init() function.
 */
 ...
 * appear in this array. For the EVB port, the Redboot partition parsing
...
static int __init mx21_mtd_init(void)
{
        ...
        if (!request_mem_region(MX21_FLASH_BASE_PA, 
                                mx21_map.size, "flash")) {
        ...
        mx21_map.virt = ioremap(MX21_FLASH_BASE_PA, mx21_map.size);
        ...
        simple_map_init(&mx21_map);

        printk(KERN_NOTICE "Probing %s at PA 0x%08lx (%d-bit)\n",
               mx21_map.name, mx21_map.phys, 
               mx21_map.bankwidth * 8);

        /*
         * Now let's probe for the actual flash.  Do it here since
         * specific machine settings might have been set above.
         */
        flash_mtd = do_map_probe("cfi_probe", &mx21_map);
        if (!flash_mtd) {
                printk("mx21 do_map_probe() failed\n");
                ret = MTD_PROBE_ERR;
                goto out_err;
        }
        ...
        if (nr_parsed_parts) {
                ...
                ret = add_mtd_partitions(flash_mtd, mx21_part, 
                                         ARRAY_SIZE(mx21_part));
        }
 ...
 * @return  0 if successful; non-zero otherwise
 */
static int __init mx21_init_maps(void)
{
	if (mx21_mtd_init() == 0) {
                return 0;
	}else {
...
static void __exit mx21_mtd_cleanup(void)
{
        if (flash_mtd) {
                ...
                iounmap((void *)mx21_map.virt);
                if (parsed_parts)
                        kfree(parsed_parts);
        }
}

module_init(mx21_init_maps);
module_exit(mx21_mtd_cleanup);

MODULE_AUTHOR("Freescale");
MODULE_DESCRIPTION("MX21 MTD map driver");
MODULE_LICENSE("GPL");
