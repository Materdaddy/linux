...
{
    tsc2100_devdata->reg.control1 &= ~0x3F;
    tsc2100_devdata->reg.control3 |= ( 1 << 13 );
    tsc2100_regwrite(TSC2100_REG_CONTROL1, tsc2100_devdata->reg.control1);
    tsc2100_regwrite(TSC2100_REG_CONTROL3, tsc2100_devdata->reg.control3);
    if( CSCR & CSCR_MCU_SEL ) { // detect clock version by looking at where CPU gets its clock
      ...
      tsc2100_regwrite(TSC2100_REG_PLL1,
		       ...
		       5 << 2);   // J value is 7 for 44.1 kHz
      tsc2100_regwrite(TSC2100_REG_PLL2,
		       6448<<2);  // D value is 5264 for 44.1 kHz
    } else {
      // we are in 12 MHz land (hardware version 1.5 and less)
      tsc2100_regwrite(TSC2100_REG_PLL1,
		       1 << 15 |  // enable PLL
...
static int snd_tsc2100_pcm_capture_trigger(snd_pcm_substream_t *substream, int cmd)
{
    ...
    case SNDRV_PCM_TRIGGER_START:
        ...
        if ((runtime->periods == 2)) {
            ...
            DBG("addr=%lx, size=%x\n", runtime->dma_addr + chip->c_per_size, chip->c_per_size);
            setup_dma_xfer(chip->c_dma,
                           (u32)&SSI1_SRX0_PHYS,
                           runtime->dma_addr + chip->c_per_size,
                           chip->c_per_size);
                                       ...
                                       int isTimer, struct pt_regs *regs)
{
	...
	TSDBG( "#irq mmode new=%d adc=%x pd=%d istimer=%d#  ", devdata->mode, devdata->reg.adc, devdata->pendown, isTimer );
    }
...
static void tsc2100_get_miscdata( struct tsc2100_data* devdata )
{
    ...
    spin_lock_irqsave(&(devdata->lock), flags); // bunnie@chumby.com: moved spinlock to outside the devdata->pendown test to prevent test-read-modify-write race condition
...
// henry@chumby.com - we still have some cases that fail, re-enabling this klugey workaround
//#if 0 // bunnie@chumby.com - if'd this out to test out the spinlock irqsave movement
	...
	if (devdata->inactive_count > 0)
	{
		...
		if (timeout_regdump)
		{
			RegDumpViaPrintk( " with interrupts stuck - please email this to bunnie@chumby.com\nalong with steps you took to make this happen",
					devdata );
		}
	...
	else
	{
		...
		TSDBG( "tsc2100_get_miscdata state==%d pd=%d flags=0x%08lx\n", state & 3, devdata->pendown, flags );
	}
//#endif // henry@chumby.com - re-enable workaround

    }
    spin_unlock_irqrestore(&devdata->lock, flags); // bunnie@chumby.com: moved unlock likewise outside thet set
}
