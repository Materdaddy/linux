 ...
 * variables
 *----------------------------------------------------------------------------*/
static struct i2c_imx_i2creg *i2c_imx_reg = (struct i2c_imx_i2creg *)IO_ADDRESS(IMX_I2C_BASE);
...
static int __init i2c_imx_init(void)
{
  	...
  	imx_gpio_mode(PD18_PF_I2C_SDA);
  	imx_gpio_mode(PD17_PF_I2C_SCL);

        /* Turn on the clock to I2C */
        PCCR0 |= PCCR0_I2C_EN;

#endif /* defined(CONFIG_MACH_IMX21) */
